...
class XXMLRPCServer:
    ...
    def add_object(self, path, obj):
        if path in self.objects:
            raise Exception(
                "An object with the path '%s' is already registered!" % path)
...
class XXMLObjectProxy:
    ...
    def __getattr__(self, method):
        ...
        def invoke(*params):
            call = ClientCall(self.service, method,
                              tuple([self.path] + list(params)))
            return call
        return invoke
...
def _call_destroyed(invisible):
    if invisible.xmlrpc_response is None:
        print("ClientCall object destroyed without waiting for response!",
              file=sys.stderr)
    invisible.destroy()
...
class ClientCall(tasks.Blocker):
    ...
    def __init__(self, service, method, params):
        ...
        weakself = weakref.ref(
            self, lambda r, i=self.invisible: _call_destroyed(i))
