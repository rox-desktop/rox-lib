...
class Context:
  ...
  def compile(self, source, options = None, c_only = 0, obj_only = 0):
    ...
    if result.num_errors > 0:
      result.c_file = None
    if result.c_file and not c_only:
...
def compile(source, options = None, c_only = 0, obj_only = 0):
  """
  ...
  return context.compile(source, options, c_only, obj_only)
...
def main(command_line = 0, c_only = 0):
  args = sys.argv[1:]
  any_failures = 0
  if command_line:
  ...
  for source in sources:
    try:
      result = context.compile(source, options, c_only = c_only)
      if result.num_errors > 0:
        any_failures = 1
    except PyrexError, e:
      print >>sys.stderr, e
...
if sys.platform == "mac":
  from Pyrex.Mac.MacSystem import c_compile, c_link
