...
def p_statement(s, level, cdef_flag = 0, visibility = 'private'):
  ...
  if cdef_flag:
    ...
    if level not in ('module', 'class', 'c_class'):
...
def p_suite(s, level = 'other', cdef_flag = 0,
  ...
  if s.sy == 'NEWLINE':
    ...
    if level in ('module', 'class', 'other'):
...
def p_c_declarator(s, empty = 0, is_type = 0):
  # If empty is true, the declarator must be
  # empty, otherwise we don't care.
  pos = s.position()
  if s.sy == '*':
    s.next()
    base = p_c_declarator(s, empty, is_type)
    ...
    if s.sy == '(':
      s.next()
      result = p_c_declarator(s, empty, is_type)
      s.expect(')')
    else:
      if s.sy == 'IDENT':
        name = s.systring
    ...
    while s.sy in ('[', '('):
      if s.sy == '[':
        ...
        s.next()
        args = p_c_arg_list(s, in_pyfunc = 0)
...
def p_c_func_or_var_declaration(s, level, pos, visibility = 'private'):
  ...
  if s.sy == ':':
    if level <> 'module':
      s.error("C function declaration not allowed here")
