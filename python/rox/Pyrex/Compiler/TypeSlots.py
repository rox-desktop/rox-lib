...
class SlotDescriptor:
  ...
  is_initialised_from_extern = 0

  def __init__(self, slot_name):
    self.slot_name = slot_name
    if slot_name in slots_initialised_from_extern:
      self.is_initialised_from_extern = 1
  
  def generate(self, scope, code):
    #code.putln("%s, /*%s*/" % (self.slot_code(scope), self.slot_name))
    if self.is_initialised_from_extern:
      value = 0
    else:
      value = self.slot_code(scope)
    code.putln("%s, /*%s*/" % (value, self.slot_name))
  
  def generate_extern_init_code(self, scope, code):
    # Some braindead compilers have trouble statically 
...
class GCDependentSlot(SlotDescriptor):
  ...
  def __init__(self, slot_name, no_gc_value, gc_value):
    SlotDescriptor.__init__(self, slot_name)
...
class GetSetSlot(SlotDescriptor):
  ...
  def slot_code(self, scope):
    #return scope.getset_table_cname # Later
    return "0"
...
def get_special_method_signature(name):
  ...
  slot = method_name_to_slot.get(name, None)
  if slot:
...
intobjargproc = Signature("TiO", "i")      # typedef int(*intobjargproc)(PyObject *, int, PyObject *);
intintobjargproc = Signature("TiiO", "i")  # typedef int(*intintobjargproc)(PyObject *, int, int, PyObject *);
intintargproc = Signature("Tii", "i")
objargfunc = Signature("TO", "O")
objobjargproc = Signature("TOO", "i")      # typedef int(*objobjargproc)(PyObject *, PyObject *, PyObject *);
...
objargproc = Signature("TO", "i")          # typedef int (*objobjproc)(PyObject *, PyObject *);
                                           # typedef int (*visitproc)(PyObject *, void *);
                                           # typedef int (*traverseproc)(PyObject *, visitproc, void *);

destructor = Signature("T", "v")           # typedef void (*destructor)(PyObject *);
# printfunc = Signature("TFi", "i")        # typedef int (*printfunc)(PyObject *, FILE *, int);
                                           # typedef PyObject *(*getattrfunc)(PyObject *, char *);
getattrofunc = Signature("TO", "O")        # typedef PyObject *(*getattrofunc)(PyObject *, PyObject *);
                                           # typedef int (*setattrfunc)(PyObject *, char *, PyObject *);
setattrofunc = Signature("TOO", "i")       # typedef int (*setattrofunc)(PyObject *, PyObject *, PyObject *);
delattrofunc = Signature("TO", "i")
...
descrsetfunc = Signature("TOO", "i")       # typedef int (*descrsetfunc) (PyObject *, PyObject *, PyObject *);
descrdelfunc = Signature("TO", "i")
initproc = Signature("T*", "i")            # typedef int (*initproc)(PyObject *, PyObject *, PyObject *);
...
slot_table = (
  ...
  MethodSlot(richcmpfunc, "tp_richcompare", "__richcmp__"), # InternalMethodSlot("tp_richcompare"),
  ...
  SyntheticSlot("tp_descr_get", ["__get__"], "0"),                 #MethodSlot(descrgetfunc, "tp_descr_get", "__get__"),
  SyntheticSlot("tp_descr_set", ["__set__", "__delete__"], "0"),   #MethodSlot(descrsetfunc, "tp_descr_set", "__set__"),
