...
class ModuleNode(Node):
  ...
  def generate_c_code(self, env, result):
    ...
    code.init_labels()
    self.generate_module_preamble(env, code)
  ...
  def internal_method_cname(self, scope, name):
    return "%s%s_%s" % (Naming.pyrex_prefix, name, scope.class_name)
  ...
  def generate_new_function(self, scope, code):
    code.putln("")
    code.putln(
      "static PyObject *%s(struct _typeobject *t, PyObject *a, PyObject *k) {"
        % self.internal_method_cname(scope, "tp_new"))
    code.putln(
      "PyObject *o = (*t->tp_alloc)(t, 0);")
    self.generate_self_cast(scope, code)
    for entry in scope.var_entries:
      if entry.type.is_pyobject:
        #code.put_init_to_py_none("p->%s" % entry.cname)
  ...
  def generate_dealloc_function(self, scope, code):
    ...
    code.putln(
      "static void %s(PyObject *o) {"
        % self.internal_method_cname(scope, "tp_dealloc"))
    self.generate_self_cast(scope, code)
    self.generate_usr_dealloc_call(scope, code)
    for entry in scope.var_entries:
      if entry.type.is_pyobject:
        code.put_xdecref("p->%s" % entry.cname)
    code.putln(
      "(*o->ob_type->tp_free)(o);")
    code.putln(
      "}")
  ...
  def generate_traverse_function(self, scope, code):
    ...
    code.putln(
      "static int %s(PyObject *o, visitproc v, void *a) {"
        % self.internal_method_cname(scope, "tp_traverse"))
    code.putln(
      "int e;")
    self.generate_self_cast(scope, code)
    for entry in scope.var_entries:
  ...
  def generate_clear_function(self, scope, code):
    ...
    code.putln(
      "static int %s(PyObject *o) {"
        % self.internal_method_cname(scope, "tp_clear"))
    self.generate_self_cast(scope, code)
    for entry in scope.var_entries:
      if entry.type.is_pyobject:
        name = "p->%s" % entry.cname
        code.put_xdecref(name)
  ...
  def generate_getitem_int_function(self, scope, code):
    ...
    code.putln(
      "static PyObject *%s(PyObject *o, int i) {" %
        self.internal_method_cname(scope, "sq_item"))
    code.putln(
  ...
  def generate_ass_subscript_function(self, scope, code):
    ...
    code.putln(
      "static int %s(PyObject *o, PyObject *i, PyObject *v) {" %
        self.internal_method_cname(scope, "mp_ass_subscript"))
    code.putln(
        "if (v) {")
    if set_entry:
      code.putln(
  ...
  def generate_ass_slice_function(self, scope, code):
    ...
    code.putln(
      "static int %s(PyObject *o, int i, int j, PyObject *v) {" %
        self.internal_method_cname(scope, "sq_ass_slice"))
    code.putln(
        "if (v) {")
    if set_entry:
      code.putln(
  ...
  def generate_getattro_function(self, scope, code):
    ...
    code.putln(
      "static PyObject *%s(PyObject *o, PyObject *n) {"
        % self.internal_method_cname(scope, "tp_getattro"))
    code.putln(
  ...
  def generate_setattro_function(self, scope, code):
    ...
    code.putln(
      "static int %s(PyObject *o, PyObject *n, PyObject *v) {" %
        self.internal_method_cname(scope, "tp_setattro"))
    if set_entry:
      code.putln(
        "if (v) return %s(o, n, v);" %
          set_entry.func_cname)
      ...
      code.putln(
        "if (v) return PyObject_GenericSetAttr(o, n, v);")
    if del_entry:
      code.putln(
        "else return %s(o, n);" %
          del_entry.func_cname)
  ...
  def generate_descr_get_function(self, scope, code):
    ...
    code.putln(
      "static PyObject *%s(PyObject *o, PyObject *i, PyObject *c) {" %
        self.internal_method_cname(scope, "tp_descr_get"))
    code.putln(
      "PyObject *r = 0;")
    code.putln(
      "if (!i) i = Py_None;")
  ...
  def generate_descr_set_function(self, scope, code):
    ...
    code.putln(
      "static int %s(PyObject *o, PyObject *i, PyObject *v) {" %
        self.internal_method_cname(scope, "tp_descr_set"))
    code.putln(
        "if (v) {")
    if user_set_entry:
      code.putln(
  ...
  def generate_const_definitions(self, env, code):
    #self.filename = env.get_filename_const(self.pos)
    code.putln("")
  ...
  def generate_module_creation_code(self, env, code):
    ...
    code.putln(
      'PyObject_SetAttrString(%s, "__builtins__", %s);' % (
        env.module_cname,
        Naming.builtins_cname))
  ...
  def generate_global_init_code(self, env, code):
    ...
    #	if entry.typeobj_is_imported:
    #		code.putln("(PyObject *)%s = PyNone; Py_INCREF(Py_None);" %
    #			entry.type.typeptr_cname)
  ...
  def generate_type_import_code(self, entry, code):
    # Generate code to import the typeobject of an
    # extension type defined in another module.
    type = entry.type
    if type.typedef_flag:
      objstruct = type.objstruct_cname
    else:
  ...
  def generate_type_ready_code(self, entry, code):
    ...
    if scope: # could be None if there was an error
      if entry.visibility <> 'extern':
        for slot in TypeSlots.slot_table:
          slot.generate_extern_init_code(scope, code)
        code.putln(
...
#	def generate_typevar_assignment_code(self, entry, code):
#		# Generate code to assign the type object of an extension
#		# type to its associated module-level variable.
#		#print "generate_typevar_assignment_code:", entry.name
#		#print "...is_foreign =", entry.is_foreign
#		#print "...typeobj_cname =", entry.type.typeobj_cname
#		#print "...var_entry =", entry.as_variable
#		var_entry = entry.as_variable
#		type = entry.type
#		typeptr_cname = type.typeptr_cname
#		if var_entry and (entry.is_foreign or type.typeobj_cname):
#			if var_entry.is_pyglobal:
#				code.putln(
#					'if (PyObject_SetAttrString(%s, "%s", (PyObject*)%s) < 0) %s' % (
#						Naming.module_cname,
#						var_entry.name,
...
#						code.error_goto(entry.pos)))
#			else:
#				code.put_incref(typeptr_cname)
#				code.put_var_decref(var_entry)
#				code.putln(
#					"%s = (PyObject*)%s;" % (
#						var_entry.cname,
#						type.typeptr_cname))
...
class CSimpleBaseTypeNode(CBaseTypeNode):
  ...
  def analyse(self, env):
    ...
    elif not self.module_path and self.name == "object":
...
class CFuncDefNode(FuncDefNode):
  ...
  def declare_arguments(self, env):
    for arg in self.entry.type.args:
      if not arg.name:
...
class DefNode(FuncDefNode):
  ...
  def generate_argument_parsing_code(self, code):
    ...
    if self.entry.signature.has_generic_args:
      ...
      if has_starargs:
        code.putln("{")
        code.put_xdecref(Naming.args_cname)
        code.put_xdecref(Naming.kwds_cname)
...
class ReturnStatNode(StatNode):
  ...
  def analyse_expressions(self, env):
    self.return_type = env.return_type
    ...
    if self.value:
      ...
      if not (env.return_type.is_void or
        env.return_type.is_pyobject):
  ...
  def generate_execution_code(self, code):
    ...
    if self.value:
      ...
      if self.value.type.is_pyobject and not self.value.is_temp:
        code.put_incref(self.value.result)
      if self.return_type.is_extension_type:
...
class AssertStatNode(StatNode):
  ...
  def analyse_expressions(self, env):
    ...
    if self.value:
      self.value.analyse_expressions(env)
