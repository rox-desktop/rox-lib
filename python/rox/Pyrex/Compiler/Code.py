...
class CCodeWriter:
  ...
  def as_pyobject(self, entry):
    #if entry.type.is_extension_type and not entry.is_self_arg:
    if not entry.is_self_arg and not entry.type.is_complete():
      return "(PyObject *)%s" % entry.cname
    else:
      return entry.cname
    
  def put_incref(self, cname):
    self.putln("Py_INCREF(%s);" % cname)
  
  def put_var_incref(self, entry):
    if entry.type.is_pyobject:
      self.putln("Py_INCREF(%s);" % self.as_pyobject(entry))
  
  def put_decref(self, cname):
    self.putln("Py_DECREF(%s);" % cname)
  
  def put_decref_clear(self, cname):
    self.putln("Py_DECREF(%s); %s = 0;" % (cname, cname))
  
  def put_xdecref(self, cname):
    self.putln("Py_XDECREF(%s);" % cname)
  
  def put_xdecref_clear(self, cname):
    self.putln("Py_XDECREF(%s); %s = 0;" % (cname, cname))

  def put_var_decref(self, entry):
    if entry.type.is_pyobject:
      self.putln("Py_DECREF(%s);" % self.as_pyobject(entry))
  
  def put_var_xdecref(self, entry):
    if entry.type.is_pyobject:
      self.putln("Py_XDECREF(%s);" % self.as_pyobject(entry))
  
  def put_var_xdecref_clear(self, entry):
    if entry.type.is_pyobject:
      self.putln("Py_XDECREF(%s); %s = 0;" % (
        self.as_pyobject(entry), entry.cname))
