...
class TestProcesses(unittest.TestCase):
    def testTmp(self):
        tmp_file = processes._Tmp('w+')
        tmp_file.write('Hello')
        print(' ', end='', file=tmp_file)
        tmp_file.flush()
        os.write(tmp_file.fileno(), b'World')
    ...
    def testInvalidCommand(self):
        try:
            ...
            assert 0
        except processes.ChildError as ex:
            pass
        else:
            assert 0
    ...
    def testStderr(self):
        try:
            pipe_through_command(
                'echo one >&2; sleep 2; echo two >&2', None, None)
        except processes.ChildError:
            pass
        else:
            assert 0
